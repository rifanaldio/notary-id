  ...
  return (
    <header className="sticky top-0 z-30 bg-brand-surface/90 backdrop-blur border-b border-brand-muted/30 dark:bg-brand-dark/90 dark:border-brand-light/10">
      <div className="mx-auto flex justify-between items-center px-4 py-3">
        ...
        <nav className="flex items-center gap-3 text-sm font-semibold text-brand-muted dark:text-brand-light">
          {/* {navItems.map((item) => (
            <NavLink
              key={item.to}
              to={item.to}
              className={({ isActive }) =>
                `px-3 py-2 rounded-lg transition ${
                  isActive
                    ? 'bg-brand-primary/10 text-brand-primary dark:text-brand-light'
                    : 'hover:bg-brand-light dark:hover:bg-brand-dark'
                }`
              }
            >
              {item.label}
